...
TARGET_MODE = "minimal" # recommended 2025 target values
DISABLE_HOLDING_LOOP = False  # Set to True to disable the holding loop in the extraction process
...
def main(extract_targets=False):
    ...
    if target_file and os.path.exists(target_file):
        ...
        ua_dict = get_ua_groups_from_parcel_file(dm.parcels_df, mode=TARGET_MODE)
    ...
    with open(f"{extraction_id}_buckets.txt", "w", encoding="utf-8") as f:
        for key, value in dm.ua_groups.items():
            f.write(f"{key} : {value}\n")
    print("Buckets prepared. Beginning sample extraction process...")

    dm.disable_holding_loop = DISABLE_HOLDING_LOOP
